...
import { useBuildingsQuery, useDeleteBuildingMutation } from "@/api/building";
...
import AddBuildingDialog from "@/app-components/forms/Building";
...
const Buildings = () => {
  ...
  const pageSize = 10;
  const { data: branches, isPending } = useBranchesQuery(0, 1000000, companyId);
  const {
    data: buildings,
    isLoading,
    refetch,
  } = useBuildingsQuery(pageNo, pageSize, companyId, branchId, debouncedSearch);
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[23px] font-bold ">Manage Buildings</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[12px]">
          {isLoading ? (
            <TableSkeleton />
          ) : buildings && buildings.content && buildings.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {buildings.content.map((building: any, index: number) => (
                  <TableRow
                    key={building.id}
                    className={`
                  ...
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {" "}
                      {building.createdAt
                        ? format(new Date(building.createdAt), "dd/MM/yyyy")
                        : "-"}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {building.branch?.branchName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
      ...
      {buildings && buildings.content && buildings.content.length > 0 && (
        <div className="pt-[30px] flex flex-row justify-center">
          <CustomPagination
            pageNo={pageNo}
            setPageNo={setPageNo}
            totalElements={buildings?.totalElements ?? 0}
            pageSize={pageSize}
          />
        </div>
      )}
