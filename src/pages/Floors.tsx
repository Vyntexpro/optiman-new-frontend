...
import { useFloorsQuery, useDeleteFloorMutation } from "@/api/floor";
...
import AddFloorDialog from "@/app-components/forms/Floor";
...
const Floors = () => {
  ...
  const pageSize = 10;
  const { data: branches, isFetching } = useBranchesQuery(
    0,
    1000000,
    companyId
  );
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[23px] font-bold ">Manage Floors</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[12px]">
          {isLoading ? (
            <TableSkeleton />
          ) : floors && floors.content && floors.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {floors.content.map((floor: any, index: number) => (
                  <TableRow
                    key={floor.id}
                    className={`
                  ...
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {floor.building?.branch?.branchName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {floor.building?.buildingName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {floor.floorName}
                    </TableCell>
      ...
      {floors && floors.content && floors.content.length > 0 && (
        <div className="pt-[30px] flex flex-row justify-center">
          <CustomPagination
            pageNo={pageNo}
            setPageNo={setPageNo}
            totalElements={floors?.totalElements ?? 0}
            pageSize={pageSize}
          />
        </div>
      )}
