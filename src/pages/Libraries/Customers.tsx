...
import { useCustomersQuery, useDeleteCustomerMutation } from "@/api/customer";
...
import AddCustomerDialog from "@/app-components/forms/Customer";
...
const Customer = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useCustomersQuery(pageNo, pageSize, companyId, debouncedSearch);
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[22px] font-bold ">Customer Libraries</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[10px]">
          {isLoading ? (
            <TableSkeleton />
          ) : customers && customers.content && customers.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {customers.content.map((customer: any, index: number) => (
                  <TableRow
                    key={customer.id}
                    className={`
                      border-b border-slate-100 last:border-b-0
                      hover:bg-slate-50 transition-customers
                    `}
                  >
