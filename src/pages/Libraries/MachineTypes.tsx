...
import { useTypesQuery, useDeleteTypeMutation } from "@/api/machineType";
...
import AddTypeDialog from "@/app-components/forms/MachineType";
...
const MachineType = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useTypesQuery(pageNo, pageSize, companyId, debouncedSearch);
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[22px] font-bold ">Machine Type Libraries</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[10px]">
          {isLoading ? (
            <TableSkeleton />
          ) : types && types.content && types.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {types.content.map((type: any, index: number) => (
                  <TableRow
                    key={type.id}
                    className={`
                  ...
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {" "}
                      {type.createdAt
                        ? format(new Date(type.createdAt), "dd/MM/yyyy")
                        : "-"}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {type.machineType}
      ...
      {types && types.content && types.content.length > 0 && (
        <div className="pt-[30px] flex flex-row justify-center">
          <CustomPagination
            pageNo={pageNo}
            setPageNo={setPageNo}
            totalElements={types?.totalElements ?? 0}
            pageSize={pageSize}
          />
        </div>
      )}
