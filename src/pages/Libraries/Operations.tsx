...
import AddOperationDialog from "@/app-components/forms/Operation";
...
const Operation = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useOperationsQuery(pageNo, pageSize, companyId, debouncedSearch);
  const deleteMutation = useDeleteOperationMutation();
  return (
    <div className="pb-10">
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[22px] font-bold ">Operation Libraries</h2>
          </div>
                ...
                {operations.content.map((operation: any, index: number) => (
                  <TableRow
                    ...
                    className={`
                      ...
                      hover:bg-slate-50 transition-operations
                    `}
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {" "}
                      {operation.createdAt
                        ? format(new Date(operation.createdAt), "dd/MM/yyyy")
                        : "-"}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
      ...
      {operations && operations.content && operations.content.length > 0 && (
        <div className="pt-[30px] flex flex-row justify-center">
          <CustomPagination
            pageNo={pageNo}
            setPageNo={setPageNo}
            totalElements={operations?.totalElements ?? 0}
            pageSize={pageSize}
          />
        </div>
      )}
