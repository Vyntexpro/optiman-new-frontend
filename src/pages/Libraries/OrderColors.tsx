...
import { useColorsQuery, useDeleteColorMutation } from "@/api/orderColor";
...
import AddOrderColorDialog from "@/app-components/forms/OrderColor";
...
const OrderColor = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useColorsQuery(pageNo, pageSize, companyId, debouncedSearch);
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[22px] font-bold ">Order Color Libraries</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[10px]">
          {isLoading ? (
            <TableSkeleton />
          ) : colors && colors.content && colors.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {colors.content.map((color: any, index: number) => (
                  <TableRow
                    key={color.id}
                    className={`
                  ...
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {" "}
                      {color.createdAt
                        ? format(new Date(color.createdAt), "dd/MM/yyyy")
                        : "-"}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {color.orderColor}
      ...
      {colors && colors.content && colors.content.length > 0 && (
        <div className="pt-[30px] flex flex-row justify-center">
          <CustomPagination
            pageNo={pageNo}
            setPageNo={setPageNo}
            totalElements={colors?.totalElements ?? 0}
            pageSize={pageSize}
          />
        </div>
      )}
