...
import { useRowsQuery, useDeleteRowMutation } from "@/api/row";
...
import AddRowDialog from "@/app-components/forms/Row";
...
const Rows = () => {
  ...
  const pageSize = 10;
  const { data: branches } = useBranchesQuery(0, 1000000, companyId);
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h2 className="text-[23px] font-bold ">Manage Rows</h2>
          </div>
        ...
        <div className="rounded-lg bg-white border border-slate-200 shadow-2xl shadow-primary/10 backdrop-blur-lg px-4 py-2 mt-[12px]">
          {isLoading ? (
            <TableSkeleton />
          ) : rows && rows.content && rows.content.length > 0 ? (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {rows.content.map((row: any, index: number) => (
                  <TableRow
                    key={row.id}
                    className={`
                  ...
                  >
                    ...
                    <TableCell className="py-3 text-[11px]">
                      {row.hall?.floor?.building?.branch?.branchName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {row.hall?.floor?.building?.buildingName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {row.hall?.floor?.floorName}
                    </TableCell>
                    <TableCell className="py-3 text-[11px]">
                      {row.hall?.hallName}
                    </TableCell>
